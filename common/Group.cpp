    ...
    for (int i=0;i<materials.size();i++)
        {
            ...
            //pass a render mode to the shaders if needed
...
void Group::setupShaders(){
	...
	for (int i=0;i<meshes.size();i++)
    {
        ...
        if(!mat->shaderIsInitialized()){
            ...
            std::cout << "SHININESS BEFORE PASSED TO SHADER: ";
            std::cout << mat->getShininess() << std::endl;
            mtlshader->setSpecularExponent(mat->getShininess());

            std::cout << "OPACITY BEFORE PASSED TO SHADER: ";
            std::cout << mat->getOpacity() << std::endl;
